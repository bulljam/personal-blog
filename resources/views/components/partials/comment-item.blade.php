...
<div
    ...
    <div class="flex items-start gap-3">
        ...
        <div class="flex-1 min-w-0">
            ...
            @auth
                @if (!$isMaxDepth && $replyingTo !== $comment->id && $editingCommentId !== $comment->id)
                ...
                @endif
                @if ($replyingTo === $comment->id || $editingCommentId === $comment->id)
        ...
        <div class="flex items-center gap-2 shrink-0">
            @if ($editingCommentId !== $comment->id && $replyingTo !== $comment->id)
                <button wire:click="editComment({{ $comment->id }}, '{{ $depth === 0 ? 'commentContent' : 'replyContent' }}')"
    ...
    @auth
        @if ($replyingTo === $comment->id)
            <div class="mt-4 ml-13">
                <form wire:submit="addComment('replyContent')" class="space-y-3">
                    <textarea wire:model="replyContent" rows="3" placeholder="Write your reply..."
        ...
        @elseif ($editingCommentId === $comment->id)
            <div class="mt-4 ml-13">
                @if ($depth === 0)
                    <form wire:submit="updateComment('commentContent')" class="space-y-3">
                        <textarea wire:model="commentContent" rows="3" placeholder="Edit your comment..."
                            class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"></textarea>
                ...
                @else
                    <form wire:submit="updateComment('replyContent')" class="space-y-3">
                        <textarea wire:model="replyContent" rows="3" placeholder="Edit your reply..."
                            class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"></textarea>
                        @error('replyContent')
                            <p class="text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
                        <div class="flex items-center gap-2">
                            <button type="submit" wire:loading.attr="disabled" wire:target="updateComment"
                                class="px-3 py-1.5 text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 rounded-lg transition-colors disabled:opacity-50">
                                <span wire:loading.remove wire:target="updateComment('replyContent')">Update Reply</span>
                                <span wire:loading wire:target="updateComment('replyContent')">Updating...</span>
                            </button>
                            <button type="button" wire:click="cancelReply"
                                class="px-3 py-1.5 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors">
                                Cancel
