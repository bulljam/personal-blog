...
<div x-data="{ open: false }" @close-modal.window="open = false">
    ...
    <div x-show="open" x-cloak x-transition>
        <x-partials.modal title="Update Password">
            <form wire:submit="updatePassword" class="space-y-4">
                <div>
                    ...
                    <input type="password" wire:model="password" @class([
                        'w-full px-3 py-2 mb-2 border rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors',
                        'border-red-500 dark:border-red-500' => $errors->has('password'),
                        'border-gray-300 dark:border-gray-700' => !$errors->has('password'),
                    ]) />
                    <!-- Error Message -->
                    @error('password')
                        <p class="my-1 text-sm text-red-600 dark:text-red-400 flex items-center gap-1.5">
                    ...
                    <input type="password" wire:model="password_confirmation" @class([
                        ...
                        'border-red-500 dark:border-red-500' => $errors->has('password_confirmation'),
                        'border-gray-300 dark:border-gray-700' => !$errors->has('password_confirmation'),
                    ]) />
                    <!-- Info Message -->
                    @session('info')
                        <div
